    ...
    <div class="pages">
        <div class="control-panel">
            <div class="page-options-btn">
                <button class="btn" type="button" @click="selectAll">SELECT ALL</button>
                <button class="btn" type="button" @click="reset">RESET</button>
                <button class="btn" :class="{'enable-btn' : isOrderLocked}"
                        type="button" @click="lockSort">ORDER LOCK
                    <prompt :prompt="orderPrompt">&#128269;</prompt>
                </button>
                <button class="btn" :class="{'enable-btn' : isDeleteEnable}"
                        @click="enableDelete">DELETE ON
                    <prompt :prompt="deletePrompt">&#128269;</prompt>
                </button>
            </div>
            <div class="page-selected">Selected pages: {{selectedPages.join(", ")}}</div>
        </div>
        <div class="pages-board">
            <div v-for="page in pages" :key="page" class="page"
                 :class="{'page-drag' : page.drag, 'page-drop' : page.drop, 'page-static' : !page.drop}">
                <div>
                    <label class="page-checkbox">
                        <input v-model="selectedPages" :value="page.number" type="checkbox"
                               class="page-check-input" required>
                    </label>
                </div>
                <div class="page-number">{{page.number}}</div>
                <div class="page-delete">
                    <button @click="deletePage(page)" class="delete-btn" type="button" title="DELETE">&#10060;
                    </button>
                </div>
                <div>
                    <img class="page-img"
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                orderPrompt: "При активации кнопка меняет цвет. Если функция включена, порядок выделения страниц - важен! Обратите внимание на порядок выделенных выделенных страниц! Также включается возможность менять страницы местами перетаскиванием.",
                deletePrompt: "При активации кнопка меняет цвет. Если функция включена, то после разделения, выбранные страницы будут удаленны из общего списка. После удаления всех страниц, они будут доступны снова.",
        ...
        updated() {
            if (this.isSuccessSplit && this.isDeleteEnable) {
                this.$emit('reset');
                this.pages = this.pages.filter(el => !this.selectedPages.includes(el.number));
                this.selectedPages = [];
        ...
        methods: {
            dragEnd(page) {//Источник перетаскивания получит событие dragEnd, когда перетаскивание завершится, было оно удачным или нет.
                //console.log(`DRAG END`);
                const index = this.pages.indexOf(page);
                this.pages[index].drag = false;
            },
            dragEnter(page) {//Срабатывает, когда перемещаемый элемент попадает на элемент-назначение
            ...
            dragStart(evt, page) {//Срабатывает когда элемент начал перемещаться
                const index = this.pages.indexOf(page);
            ...
            onDrop(evt, currentPage) {//вызывается для элемента, над которым произошло "сбрасывание" перемещаемого элемента.
                const dragPageNumber = evt.dataTransfer.getData('pageNum');
                const dragPage = this.pages.find(page => page.number === parseInt(dragPageNumber));
                ...
                const currentPageNumberIndex = this.selectedPages.indexOf(currentPage.number);
                if (dragPageNumberIndex !== -1 && currentPageNumberIndex !== -1) {
